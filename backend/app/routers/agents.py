...
from app.services import STAFF_ROLES, cache
...
# Phase 5: short-TTL cache for the operator monitoring page.
# The window is keyed into the cache key so a 1h and a 24h
# query don't collide. 15s is the right shape: long enough
# that 20 concurrent operators stop re-aggregating
# agent_logs, short enough that "an agent just went down"
# is visible within a minute.
AGENT_METRICS_TTL = 15
...
async def agent_metrics(
    ...
    key = f"agents:metrics:{window_minutes}"
    return await cache.get_or_set(key, AGENT_METRICS_TTL, _load)
